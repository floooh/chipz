...
pub fn Z80(comptime P: Pins, comptime Bus: anytype) type {
    ...
    return struct {
        ...
        // BEGIN CONSTS
        const M1_T2: u16 = 0x740;
        const M1_T3: u16 = 0x741;
        const M1_T4: u16 = 0x742;
        const DDFD_M1_T2: u16 = 0x743;
        const DDFD_M1_T3: u16 = 0x744;
        const DDFD_M1_T4: u16 = 0x745;
        const DDFD_D_T1: u16 = 0x746;
        const DDFD_D_T2: u16 = 0x747;
        const DDFD_D_T3: u16 = 0x748;
        const DDFD_D_T4: u16 = 0x749;
        const DDFD_D_T5: u16 = 0x74A;
        const DDFD_D_T6: u16 = 0x74B;
        const DDFD_D_T7: u16 = 0x74C;
        const DDFD_D_T8: u16 = 0x74D;
        const DDFD_LDHLN_WR_T1: u16 = 0x74E;
        const DDFD_LDHLN_WR_T2: u16 = 0x74F;
        const DDFD_LDHLN_WR_T3: u16 = 0x750;
        const DDFD_LDHLN_OVERLAPPED: u16 = 0x751;
        const ED_M1_T2: u16 = 0x752;
        const ED_M1_T3: u16 = 0x753;
        const ED_M1_T4: u16 = 0x754;
        // END CONSTS
        ...
        pub fn tick(self: *Self, in_bus: Bus) Bus {
            ...
            next: {
                switch (self.step) {
                    ...
                    0x5A5 => {
                        self.r[B] = self.in(self.dlatch);
                    },
                    // OUT (C),B (continued...)
                    0x5A6 => {
                        bus = iowr(bus, self.BC(), self.r[B]);
                        self.step = 0x5A7;
                        break :next;
                    },
                    0x5A7 => {
                    ...
                    0x5C7 => {
                        self.r[C] = self.in(self.dlatch);
                    },
                    // OUT (C),C (continued...)
                    0x5C8 => {
                        bus = iowr(bus, self.BC(), self.r[C]);
                        self.step = 0x5C9;
                        break :next;
                    },
                    0x5C9 => {
                    ...
                    0x5E9 => {
                        self.r[D] = self.in(self.dlatch);
                    },
                    // OUT (C),D (continued...)
                    0x5EA => {
                        bus = iowr(bus, self.BC(), self.r[D]);
                        self.step = 0x5EB;
                        break :next;
                    },
                    0x5EB => {
                    ...
                    0x60B => {
                        self.r[E] = self.in(self.dlatch);
                    },
                    // OUT (C),E (continued...)
                    0x60C => {
                        bus = iowr(bus, self.BC(), self.r[E]);
                        self.step = 0x60D;
                        break :next;
                    },
                    0x60D => {
                    ...
                    0x62D => {
                        self.r[H] = self.in(self.dlatch);
                    },
                    // OUT (C),H (continued...)
                    0x62E => {
                        bus = iowr(bus, self.BC(), self.r[H]);
                        self.step = 0x62F;
                        break :next;
                    },
                    0x62F => {
                    ...
                    0x64D => {
                        self.dlatch = self.rrd(self.dlatch);
                        self.step = 0x64E;
                        break :next;
                    },
                    0x64E => {
                    ...
                    0x658 => {
                        self.r[L] = self.in(self.dlatch);
                    },
                    // OUT (C),L (continued...)
                    0x659 => {
                        bus = iowr(bus, self.BC(), self.r[L]);
                        self.step = 0x65A;
                        break :next;
                    },
                    0x65A => {
                    ...
                    0x678 => {
                        self.dlatch = self.rld(self.dlatch);
                        self.step = 0x679;
                        break :next;
                    },
                    0x679 => {
                    ...
                    0x683 => {
                        _ = self.in(self.dlatch);
                    },
                    // OUT (C) (continued...)
                    0x684 => {
                        bus = iowr(bus, self.BC(), 0);
                        self.step = 0x685;
                        break :next;
                    },
                    0x685 => {
                    ...
                    0x695 => {
                        if (wait(bus)) break :next;
                        bus = mwr(bus, self.WZ(), self.r[SPL]);
                        self.incWZ();
                        self.step = 0x696;
                        break :next;
                    },
                    ...
                    0x698 => {
                        if (wait(bus)) break :next;
                        bus = mwr(bus, self.WZ(), self.r[SPH]);
                        self.step = 0x699;
                        break :next;
                    },
                    0x699 => {
                    ...
                    0x6A4 => {
                        self.r[A] = self.in(self.dlatch);
                    },
                    // OUT (C),A (continued...)
                    0x6A5 => {
                        bus = iowr(bus, self.BC(), self.r[A]);
                        self.step = 0x6A6;
                        break :next;
                    },
                    0x6A6 => {
                    ...
                    0x711 => {
                        if (wait(bus)) break :next;
                        bus = mwr(bus, self.HL(), self.dlatch);
                        self.incHL(); if (self.gotoFalse(self.iniind(self.r[C] +% 1), 0x712 + 5)) break :next;
                        self.step = 0x712;
                        break :next;
                    },
                    0x712 => {
                    ...
                    0x71E => {
                        if (self.gotoFalse(self.ldildd(), 0x71F + 5)) break :next;
                        self.step = 0x71F;
                        break :next;
                    },
                    0x71F => {
                    ...
                    0x728 => {
                        self.decWZ(); if (self.gotoFalse(self.cpicpd(), 0x729 + 5)) break :next;
                        self.step = 0x729;
                        break :next;
                    },
                    0x729 => {
                    ...
                    0x738 => {
                        if (wait(bus)) break :next;
                        bus = mwr(bus, self.HL(), self.dlatch);
                        self.decHL(); if (self.gotoFalse(self.iniind(self.r[C] -% 1), 0x739 + 5)) break :next;
                        self.step = 0x739;
                        break :next;
                    },
                    0x739 => {
